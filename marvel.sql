...
INSERT INTO movies (title, year, show_time) VALUES ('Avengers: Infinity War', 2018, '2:45');

UPDATE people SET name = 'Vicky Jackson'
WHERE name = 'Vicky Jackson-Five';
DELETE FROM movies WHERE (title, year, show_time) = ('Batman Begins', 2005, '12:30');
INSERT INTO people (name) VALUES ('Keith Douglas');
DELETE FROM people WHERE (name) = ('Donald Trump');

DELETE FROM movies WHERE title IN ('Ant-Man', 'Guardians of the Galaxy',
'Black Panther');
